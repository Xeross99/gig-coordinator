...
class ReservationServiceTest < ActiveSupport::TestCase
  ...
  test "seed_on_create reserves ONLY users of the currently highest rank (no cascade)" do
    ...
    # Only ala is mistrz_piora — she's the lone top-tier user, so only she is reserved,
  ...
  test "seed_on_create invites every user at the top tier" do
    # Two users tied at mistrz_piora — both should get reservations.
  ...
  test "seed_on_create creates zero reservations when no mistrz_piora exists" do
  ...
  test "refill_one skips blocked users when inviting a replacement mistrz_piora" do
  ...
  test "seed_on_create skips users with a HostBlock (defense-in-depth filter)" do
    # Kontrakt: HostBlock nie może istnieć dla mistrz_piora (walidacja + callback
  ...
  test "seed_on_create creates reservations for top-tier invitees" do
    users(:bartek).update!(title: :mistrz_piora)  # 2 top-tier invitees
    event = build_event(capacity: 3)

    ReservationService.seed_on_create(event)
    assert_equal 2, event.participations.reserved.count
  end

  test "refill_one does NOT cascade down tiers — slot stays open when no top-tier user is available" do
    event = build_event(capacity: 1)
    ReservationService.seed_on_create(event)
    # ala (mistrz_piora) is the sole top-tier user. She declines → cancelled.
    # No other user has title=mistrz_piora, so the slot must stay empty —
    # NOT fall through to bartek (kurnikowy_gangster).
  ...
  test "refill_one invites another top-tier user when one is still available" do
    # Two users at mistrz_piora. One gets a reservation and cancels; refill_one
  ...
  test "refill_one promotes from waitlist FIRST, before inviting a new top-tier candidate" do
    ...
    users(:bartek).update!(title: :mistrz_piora)  # make bartek a top-tier candidate
    ...
    # Free one confirmed slot. skip_refill wyłącza auto-refill z callbacku —
    # ten test sprawdza refill_one jako jednostkę.
    ala_p = event.participations.find_by(user: users(:ala))
    ala_p.skip_refill = true
    ala_p.update!(status: :cancelled)
  ...
  test "komendant NEVER gets auto-reservations, even without any mistrz_piora" do
    # Nowa reguła: tylko mistrz_piora dostaje rezerwacje. Komendant, mimo że
  ...
  test "mistrz_piora wygrywa z komendantem przy seed_on_create" do
  ...
  # the fly as simple zoltodziobs so the existing fixture ranks don't interfere.
  ...
  test "decreasing capacity demotes the most-recent confirmed back onto the waitlist" do
    event = event_with_roster(capacity: 4, confirmed: 4, waitlist: 0)

    event.update!(capacity: 2)

    # 2 najnowszych confirmed (najwyższe pozycje) trafia na waitlistę.
    assert_equal 2, event.participations.confirmed.count
    assert_equal 2, event.participations.waitlist.count
  end

  # ---- Zaproszenia ponad capacity ------------------------------------------
  # Każdy mistrz_piora dostaje zaproszenie niezależnie od capacity. Kto
  # pierwszy kliknie „Akceptuję" zajmuje slot, kolejni lecą na waitlistę.

  test "seed_on_create invites EVERY mistrz_piora even when their count exceeds capacity" do
    users(:bartek).update!(title: :mistrz_piora)
    users(:cezary).update!(title: :mistrz_piora)
    # ala + bartek + cezary = 3 mistrzów. Capacity = 1.
    event = build_event(capacity: 1)

    ReservationService.seed_on_create(event)

    reserved_users = event.participations.reserved.includes(:user).map(&:user).to_set
    assert_equal 3, event.participations.reserved.count,
                 "wszyscy mistrzowie pióra muszą dostać zaproszenie, niezależnie od capacity"
    assert_equal Set[users(:ala), users(:bartek), users(:cezary)], reserved_users
  end

  test "seed_on_create creates reservations for every mistrz, not capped at capacity" do
    users(:bartek).update!(title: :mistrz_piora)
    users(:cezary).update!(title: :mistrz_piora)
    event = build_event(capacity: 1)
  ...
  # ---- Pozycje rezerwacji uwzględniają confirmed -------------------------

  test "invite! assigns position after existing confirmed participants" do
    users(:bartek).update!(title: :mistrz_piora)
    event = build_event(capacity: 6)

    # Two confirmed participants already on the event.
    u1 = User.create!(first_name: "C0", last_name: "User", email: "c0@test.example")
    u2 = User.create!(first_name: "C1", last_name: "User", email: "c1@test.example")
    event.participations.create!(user: u1, status: :confirmed, position: 1)
    event.participations.create!(user: u2, status: :confirmed, position: 2)

    ReservationService.seed_on_create(event)

    reserved = event.participations.reserved.order(:position)
    assert_equal 2, reserved.size

    # Reservations must come AFTER existing confirmed (positions 3 and 4).
    assert reserved.all? { |r| r.position > 2 },
           "reserved positions (#{reserved.pluck(:position)}) must be > 2 (max confirmed position)"
    assert_equal [ 3, 4 ], reserved.pluck(:position)
  end

  test "accepted reservation lands in correct position among confirmed" do
    users(:bartek).update!(title: :mistrz_piora)
    event = build_event(capacity: 6)

    # Confirmed at position 1.
    u1 = User.create!(first_name: "C0", last_name: "User", email: "c0@test.example")
    event.participations.create!(user: u1, status: :confirmed, position: 1)

    ReservationService.seed_on_create(event)

    # Another user joins confirmed while reservations are pending.
    u2 = User.create!(first_name: "C1", last_name: "User", email: "c1@test.example")
    pos = (event.participations.holding_slot.maximum(:position) || 0) + 1
    event.participations.create!(user: u2, status: :confirmed, position: pos)

    # Accept ala's reservation (she was first reserved, position should be 2).
    ala_res = event.participations.reserved.find_by(user: users(:ala))
    ala_res.update!(status: :confirmed, reserved_until: nil)

    order = event.participations.confirmed.order(:position).map(&:user)
    assert_equal u1, order[0], "C0 should be first (position 1)"
    assert_equal users(:ala), order[1], "ala should be second (position 2, from reservation)"
  end

  test "refill_one na sub-evencie cyklu zaprasza mistrzów normalnie" do
    users(:bartek).update!(title: :mistrz_piora)
    campaign = EventCampaign.create!(host: @host, name: "Cykl", capacity: 4)
    ...
    sub = Event.create!(
      ...
      pay_per_person: 100, capacity: 1
    )
    sub.participations.destroy_all
    ReservationService.refill_one(sub)

    assert_equal 1, sub.participations.reserved.count,
                 "sub-event zaprasza mistrzów niezależnie od kampanii"
  end

  test "refill_one na sub-evencie wciąż promuje z waitlisty (cycle members)" do
    campaign = EventCampaign.create!(host: @host, name: "Cykl", capacity: 4)
    campaign.campaign_participations.destroy_all
    ...
    sub.participations.destroy_all
    Participation.create!(event: sub, user: users(:cezary), status: :waitlist, position: 1)

    ReservationService.refill_one(sub)
    assert_equal "confirmed", sub.participations.find_by(user: users(:cezary)).status,
                 "promocja z waitlisty działa nadal na sub-evencie"
  end
end
