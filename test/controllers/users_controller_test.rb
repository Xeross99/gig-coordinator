...
class UsersControllerTest < ActionDispatch::IntegrationTest
  test "routes expose admin CRUD URLs for users" do
    assert_equal "/kurolapacze/nowy",           new_user_path
    assert_equal "/kurolapacze/1/edytuj",       edit_user_path(1)
  end
  ...
  test "GET /kurolapacze/:id requires login" do
    get user_path(users(:ala))
    assert_redirected_to login_path
  end

  test "GET /kurolapacze/:id as host redirects (worker-facing area)" do
    sign_in_as(hosts(:jan))
    get user_path(users(:ala))
    assert_redirected_to login_path
  end

  test "GET /kurolapacze/:id shows the user's display_name, email, rank, and catches count" do
  ...
  test "GET /kurolapacze/:id lists upcoming participations" do
    event = events(:chickens_tomorrow)
  ...
  test "GET /kurolapacze/:id 404s for a non-existent user" do
    sign_in_as(users(:ala))
    get user_path(id: 999_999)
    assert_response :not_found
  end

  test "GET /kurolapacze/:id shows 'Zarządza' section with linked hosts for a komendant" do
  ...
  test "GET /kurolapacze/:id hides 'Zarządza' section when user has no managed_hosts" do
    sign_in_as(users(:bartek))
    get user_path(users(:cezary))
    assert_no_match Copy::Users::MANAGES, response.body
  end

  test "GET /kurolapacze/:id shows 'Zarządza wszystkimi' section with ALL hosts for mistrz_piora" do
  ...
  test "GET /kurolapacze/nowy requires login" do
    get new_user_path
    assert_redirected_to login_path
  end

  test "GET /kurolapacze/nowy as non-admin user redirects with alert" do
  ...
  test "GET /kurolapacze/nowy as host redirects (hosts can't be admins)" do
    sign_in_as(hosts(:jan))
    get new_user_path
    assert_redirected_to login_path
  end

  test "GET /kurolapacze/nowy as admin renders form" do
  ...
  test "POST /kurolapacze as admin creates user and redirects to show" do
    sign_in_as(users(:ala))
    assert_difference -> { User.count }, 1 do
      post users_path, params: { user: {
        first_name: "Nowy", last_name: "Kurołapacz",
        email: "nowy@example.com", title: "zoltodziob"
      } }
    end
    created = User.find_by(email: "nowy@example.com")
    assert_redirected_to user_path(created)
    assert_equal "Dodano kurołapacza.", flash[:notice]
  end

  test "POST /kurolapacze as non-admin is rejected" do
    sign_in_as(users(:bartek))
  ...
  test "POST /kurolapacze cannot set admin=true via params" do
    sign_in_as(users(:ala))
    post users_path, params: { user: {
  ...
  test "GET /kurolapacze/:id/edytuj as admin renders form" do
    sign_in_as(users(:ala))
  ...
  test "GET /kurolapacze/:id/edytuj as non-admin redirects" do
    sign_in_as(users(:bartek))
    get edit_user_path(users(:cezary))
    assert_redirected_to root_path
  end

  test "PATCH /kurolapacze/:id as admin updates fields and redirects to show" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /kurolapacze/:id cannot flip admin flag via params" do
    sign_in_as(users(:ala))
    patch user_path(users(:bartek)), params: { user: { admin: true } }
    assert_equal false, users(:bartek).reload.admin
  end

  test "PATCH /kurolapacze/:id as admin flips can_drive" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /kurolapacze/:id as non-admin cannot change can_drive" do
  ...
  test "GET /kurolapacze/:id pokazuje 'Kierowca: Tak/Nie' zgodnie z can_drive" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /kurolapacze/:id with invalid email re-renders form" do
    sign_in_as(users(:ala))
    patch user_path(users(:bartek)), params: { user: { email: "nie-email" } }
    assert_response :unprocessable_content
  end

  test "POST /kurolapacze with duplicate first+last name pair as admin re-renders form" do
  ...
  test "POST /kurolapacze as admin saves phone when provided" do
    sign_in_as(users(:ala))
    post users_path, params: { user: {
  ...
  test "PATCH /kurolapacze/:id as admin updates phone" do
    sign_in_as(users(:ala))
    patch user_path(users(:bartek)), params: { user: { phone: "500 600 700" } }
    assert_equal "500 600 700", users(:bartek).reload.phone
  end

  test "PATCH /kurolapacze/:id as admin can clear phone by sending blank" do
  ...
  test "PATCH /kurolapacze/:id as admin replaces managed_host_ids" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /kurolapacze/:id as admin can clear managed_host_ids by sending empty" do
    sign_in_as(users(:ala))
    users(:bartek).managed_hosts << hosts(:jan)
  ...
  test "PATCH /kurolapacze/:id as admin adds a HostBlock via blocked_host_ids" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /kurolapacze/:id as admin can remove a HostBlock" do
    sign_in_as(users(:ala))
    users(:bartek).update!(title: :kurzy_pacholek)
  ...
  test "PATCH /kurolapacze/:id ignores blocked_host_ids when title becomes mistrz_piora" do
    sign_in_as(users(:ala))
    users(:bartek).update!(title: :kurzy_pacholek)
    patch user_path(users(:bartek)), params: { user: {
      title: "mistrz_piora",
  ...
  test "GET /kurolapacze/:id/edytuj renders managed + blocked checkbox sections" do
    sign_in_as(users(:ala))
    users(:bartek).update!(title: :kurzy_pacholek)
    get edit_user_path(users(:bartek))
    assert_response :success
  ...
  test "GET /kurolapacze/:id/edytuj hides blocked checkboxes for mistrz_piora" do
    sign_in_as(users(:ala))
  ...
  test "GET /kurolapacze/:id renders phone as tel: link when present" do
  ...
  test "GET /kurolapacze/:id shows 'Nie podano' for phone when blank" do
  ...
  test "GET /kurolapacze/:id renders email as mailto: link" do
    sign_in_as(users(:cezary))
    get user_path(users(:bartek))
    assert_response :success
  ...
  test "GET /kurolapacze/:id/edytuj wires the photo-upload Stimulus controller" do
    sign_in_as(users(:ala))
    get edit_user_path(users(:bartek))
    assert_response :success
  ...
  test "PATCH /kurolapacze/:id attaches a photo from a direct-upload signed_id" do
    sign_in_as(users(:ala))
  ...
  test "POST /kurolapacze/:id/testowy-push as admin enqueues :test push and redirects with notice" do
    target = users(:bartek)
  ...
  test "POST testowy-push alerts when the user has no push subscription at all" do
    ...
    assert_redirected_to user_path(target)
    assert_equal "Ten kurołapacz nie ma żadnej subskrypcji push - powiadomienie nie miałoby gdzie dotrzeć.", flash[:alert]
  end
  ...
  # --- usuwanie kurołapacza (regresja: FK constraint -> 500) -------------------

  test "DELETE /kurolapacze/:id usuwa usera z pelnym bagazem asocjacji" do
    host   = hosts(:jan)
    ...
    assert_equal "Usunięto kurołapacza.", flash[:notice]
