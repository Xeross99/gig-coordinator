...
class HostsControllerTest < ActionDispatch::IntegrationTest
  test "routes expose admin CRUD URLs for hosts" do
    assert_equal "/gospodarze/1",               host_path(1)
    assert_equal "/gospodarze/nowy",            new_host_path
    assert_equal "/gospodarze/1/edytuj",        edit_host_path(1)
  end
  ...
  test "GET index shows the supporter badge only for supporter hosts" do
    hosts(:jan).update!(supporter: true)

    sign_in_as(users(:ala))
    get hosts_path
    assert_response :success
    assert_select "span", text: /Wspierający/, count: 1
  end

  test "GET show renders the supporter badge for a supporter host" do
    hosts(:anna).update!(supporter: true)

    sign_in_as(users(:ala))
    get host_path(hosts(:anna))
    assert_match "Wspierający", response.body

    get host_path(hosts(:jan))
    assert_no_match "Wspierający", response.body
  end
  ...
  test "GET index uses Polish singular when a host has exactly 1 upcoming event" do
    # Fixtures: jan owns chickens_tomorrow (1), anna owns harvest_next_week (1).
    sign_in_as(users(:ala))
    get hosts_path
    assert_match "zlecenie", response.body
    assert_no_match(/zlecenia\b/, response.body)
    assert_no_match(/zleceń\b/,   response.body)
  end
  ...
  test "GET index uses Polish 2-4 plural when a host has 2 upcoming events" do
    ...
    sign_in_as(users(:ala))
    get hosts_path
    assert_match(/zlecenia\b/, response.body)  # jan: 2
    assert_match "zlecenie",   response.body   # anna still has 1
  end
  ...
  test "GET index uses Polish >=5 plural when a host has 5 upcoming events" do
    ...
    # Note: jan started with 1 (chickens_tomorrow) → now 6.

    sign_in_as(users(:ala))
    get hosts_path
    assert_match(/zleceń\b/, response.body)
  end

  # --- "Zarządzam" filter (komendant-only) -----------------------------------

  test "GET /gospodarze does NOT render 'Zarządzam' toggle for zoltodziob" do
    sign_in_as(users(:bartek))  # default title zoltodziob
    get hosts_path
    assert_response :success
    assert_no_match "Zarządzam", response.body
  end

  test "GET /gospodarze does NOT render 'Zarządzam' toggle for mistrz_piora" do
  ...
  test "GET /gospodarze does NOT render 'Zarządzam' toggle for komendant with zero managed hosts" do
  ...
  test "GET /gospodarze renders 'Zarządzam' pill for komendant managing >=1 host" do
    users(:cezary).update!(title: :kurnikowy_komendant)
  ...
  test "GET /gospodarze renders only sort pills (no 'Zarządzam') for non-komendant" do
  ...
  test "GET /gospodarze?filter=managed as komendant returns only their managed hosts" do
    users(:cezary).update!(title: :kurnikowy_komendant)
    users(:cezary).managed_hosts << hosts(:jan)
  ...
  test "GET /gospodarze?filter=all as komendant returns every host" do
    users(:cezary).update!(title: :kurnikowy_komendant)
    users(:cezary).managed_hosts << hosts(:jan)
  ...
  test "GET /gospodarze (no filter) defaults to all hosts even for komendant" do
    users(:cezary).update!(title: :kurnikowy_komendant)
    users(:cezary).managed_hosts << hosts(:jan)
  ...
  test "GET /gospodarze?filter=managed by a non-komendant is ignored (returns all hosts)" do
    # bartek is zoltodziob — even if he crafts ?filter=managed manually, the
  ...
  test "GET /gospodarze with unknown filter value falls back to all" do
    users(:cezary).update!(title: :kurnikowy_komendant)
    users(:cezary).managed_hosts << hosts(:jan)
  ...
  test "GET /gospodarze as komendant: sort pills always link to filter=all, Zarządzam pill to filter=managed" do
  ...
  test "GET /gospodarze/:id requires login" do
    get host_path(hosts(:jan))
    assert_redirected_to login_path
  end

  test "GET /gospodarze/:id as signed-in user renders host details" do
  ...
  test "GET /gospodarze/:id shows 'Komendanci' section when host has managers" do
    hosts(:jan).managers << users(:cezary)
    users(:cezary).update!(title: :kurnikowy_komendant)

    sign_in_as(users(:bartek))
    get host_path(hosts(:jan))
    assert_match "Komendanci", response.body
    assert_match users(:cezary).display_name, response.body
  end

  test "GET /gospodarze/:id hides 'Komendanci' section when host has no managers" do
    sign_in_as(users(:bartek))
    get host_path(hosts(:jan))
    assert_no_match "Komendanci", response.body
  end

  test "GET /gospodarze does NOT show komendant/blokada captions (those belong on show page)" do
    hosts(:jan).managers << users(:cezary)
    users(:cezary).update!(title: :kurnikowy_komendant)
  ...
  test "GET /gospodarze links each host to show page" do
    sign_in_as(users(:bartek))
    get hosts_path
    assert_select "a[href=?]", host_path(hosts(:jan))
  end

  test "GET /gospodarze/:id as non-admin does not show edit link" do
    sign_in_as(users(:bartek))
    get host_path(hosts(:jan))
    assert_no_match %r{/gospodarze/#{hosts(:jan).id}/edytuj}, response.body
  end

  test "GET /gospodarze/:id as admin shows edit link" do
    sign_in_as(users(:ala))  # admin
    get host_path(hosts(:jan))
    assert_match %r{/gospodarze/#{hosts(:jan).id}/edytuj}, response.body
  end

  # --- admin CRUD ------------------------------------------------------------

  test "GET /gospodarze/nowy as non-admin redirects" do
    sign_in_as(users(:bartek))
    get new_host_path
  ...
  test "GET /gospodarze/nowy as admin renders form" do
    sign_in_as(users(:ala))
    get new_host_path
    assert_response :success
  ...
  test "POST /gospodarze as admin creates host and redirects to show" do
    ...
    assert_difference -> { Host.count }, 1 do
      post hosts_path, params: { host: {
        first_name: "Nowy", last_name: "Gospodarz",
    ...
    assert_equal "Dodano gospodarza.", flash[:notice]
  end

  test "POST /gospodarze as non-admin is rejected" do
    sign_in_as(users(:bartek))
  ...
  test "PATCH /gospodarze/:id as admin updates fields" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /gospodarze/:id as admin toggles the supporter flag" do
    sign_in_as(users(:ala))
    patch host_path(hosts(:jan)), params: { host: { supporter: "1" } }
    assert hosts(:jan).reload.supporter?

    patch host_path(hosts(:jan)), params: { host: { supporter: "0" } }
    assert_not hosts(:jan).reload.supporter?
  end

  test "PATCH /gospodarze/:id as non-admin is rejected" do
    sign_in_as(users(:bartek))
  ...
  test "PATCH /gospodarze/:id with invalid email re-renders form" do
    sign_in_as(users(:ala))
  ...
  test "POST /gospodarze as admin saves phone when provided" do
    sign_in_as(users(:ala))
    post hosts_path, params: { host: {
  ...
  test "PATCH /gospodarze/:id as admin updates phone" do
    sign_in_as(users(:ala))
    patch host_path(hosts(:jan)), params: { host: { phone: "600 700 800" } }
    assert_equal "600 700 800", hosts(:jan).reload.phone
  end

  test "PATCH /gospodarze/:id as admin can clear phone by sending blank" do
  ...
  test "GET /gospodarze/:id renders phone as tel: link when present" do
  ...
  test "GET /gospodarze/:id shows 'Nie podano' for phone when blank" do
  ...
  test "GET /gospodarze/:id renders email as mailto: link when present" do
    sign_in_as(users(:bartek))
    get host_path(hosts(:jan))
    assert_response :success
  ...
  test "GET /gospodarze/:id shows 'Nie podano' instead of mailto when host has no email" do
  ...
  test "GET /gospodarze/:id/edytuj wires the photo-upload Stimulus controller" do
    sign_in_as(users(:ala))
  ...
  test "PATCH /gospodarze/:id attaches a photo from a direct-upload signed_id" do
    sign_in_as(users(:ala))
