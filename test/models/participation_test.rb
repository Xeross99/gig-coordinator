...
class ParticipationTest < ActiveSupport::TestCase
  ...
  test "create as waitlist logs :joined_waitlist" do
    p = Participation.create!(event: @event, user: @user, status: :waitlist, position: 1)
    assert_equal %i[joined_waitlist], event_types(p)
  end
  ...
  test "waitlist -> confirmed logs :promoted" do
    ...
    assert_equal %i[joined_waitlist promoted], event_types(p)
  end
  ...
  test "waitlist -> cancelled logs :cancelled" do
    p = Participation.create!(event: @event, user: @user, status: :waitlist, position: 1)
    p.update!(status: :cancelled)
    assert_equal %i[joined_waitlist cancelled], event_types(p)
  end
  ...
  test "cancelled -> waitlist logs :joined_waitlist (re-activation onto waitlist)" do
    p = Participation.create!(event: @event, user: @user, status: :confirmed, position: 1)
    p.update!(status: :cancelled)
    p.update!(status: :waitlist, position: 1)
    assert_equal %i[joined cancelled joined_waitlist], event_types(p)
  end
