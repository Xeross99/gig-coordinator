...
class CarpoolRequestTest < ActiveSupport::TestCase
  ...
  test "non-participant cannot request a ride" do
    ...
    refute req.valid?
    assert req.errors[:base].any? { |m| m.include?("głównej listy") }
  end

  test "waitlist participant cannot request a ride (carpool tylko dla głównej listy)" do
    waitlister = users(:cezary)
    Participation.create!(event: @event, user: waitlister, status: :waitlist, position: 99)
    req = CarpoolRequest.new(carpool_offer: @offer, user: waitlister)
    refute req.valid?
    assert req.errors[:base].any? { |m| m.include?("głównej listy") }
  end
  ...
  test "accept! potwierdza pending zapytanie gdy są wolne miejsca" do
    req = CarpoolRequest.create!(carpool_offer: @offer, user: @passenger, status: :pending)
    req.accept!
    assert req.reload.accepted?
  end

  test "accept! przy pełnym aucie podnosi RecordInvalid i nie zmienia statusu" do
    @event.update!(capacity: 10) # żeby zmieścić pasażerów w zapisach
    4.times do |i|
      u = User.create!(first_name: "Kom#{i}", last_name: "Plet#{i}", email: "kom#{i}@example.com")
      Participation.create!(event: @event, user: u, status: :confirmed, position: 40 + i)
      CarpoolRequest.create!(carpool_offer: @offer, user: u, status: :accepted)
    end
    assert @offer.reload.full?

    extra_user = User.create!(first_name: "Nad", last_name: "Komplet", email: "nad@example.com")
    Participation.create!(event: @event, user: extra_user, status: :confirmed, position: 50)
    extra = CarpoolRequest.create!(carpool_offer: @offer, user: extra_user, status: :pending)

    error = assert_raises(ActiveRecord::RecordInvalid) { extra.accept! }
    assert error.record.errors[:base].any? { |m| m.include?("wolnych miejsc") }
    assert extra.reload.pending?
  end
  ...
  test "a driver on the event cannot request a ride from another driver" do
    ...
    req = CarpoolRequest.new(carpool_offer: @offer, user: @passenger)
    refute req.valid?
    assert req.errors[:base].any? { |m| m.include?("kierowcą na tym zleceniu") },
           "spodziewany blad 'kierowcą na tym zleceniu', mam: #{req.errors[:base].inspect}"
  end
