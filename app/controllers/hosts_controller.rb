class HostsController < ApplicationController
  ...
  def index
    ...
    if @filter == "managed" && Current.user&.kurnikowy_komendant?
  ...
  def create
    ...
    if @host.save
      redirect_to host_path(@host), notice: "Dodano gospodarza."
    else
  ...
  def update
    ...
    if @host.update(host_params)
      redirect_to host_path(@host), notice: "Zapisano zmiany."
    else
  ...
  def host_params
    params.expect(host: %i[first_name last_name email phone location supporter photo])
  end
end
