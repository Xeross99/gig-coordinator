class LoginCodesController < ApplicationController
  ...
  def create
    ...
    redirect_to verify_login_path, notice: "Jeśli adres jest w systemie, wysłaliśmy kod logowania."
  end
  ...
  def verify
    ...
    if record && LoginCode.consume(record, submitted)
      sign_in!(record)
    ...
    @email = email
    flash.now[:alert] = "Nieprawidłowy lub wygasły kod."
  ...
  def find_record(email)
    ...
    # User.enabled — wyłączone konto zachowuje się jak nieznany email
    # (neutralna odpowiedź, zero kodu, zero maila).
    Host.find_by(email: email) || User.enabled.find_by(email: email)
  end
